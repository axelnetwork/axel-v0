...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("AXEL_Utility_TokenMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("AXEL_Utility_TokenMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("AXEL_Utility_TokenMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("axel_utility_token-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running AXEL_Utility_TokenMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
