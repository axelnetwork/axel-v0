...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::AXEL)
    {
    }
                                              ...
                                              filter(0)
{
	...
	ui->pushButton_Website->setStatusTip(tr("AXEL Website"));
	ui->pushButton_Discord->setIcon(QIcon(":/icons/discord"));
	ui->pushButton_Discord->setStatusTip(tr("AXEL Discord"));
	//ui->pushButton_Telegram->setIcon(QIcon(":/icons/telegram"));
	//ui->pushButton_Telegram->setStatusTip(tr("AXEL Telegram"));
	ui->pushButton_Facebook->setIcon(QIcon(":/icons/facebook"));
	ui->pushButton_Facebook->setStatusTip(tr("AXEL Facebook"));
	ui->pushButton_Twitter->setIcon(QIcon(":/icons/twitter"));
	ui->pushButton_Twitter->setStatusTip(tr("AXEL Twitter"));
	ui->pushButton_Explorer->setIcon(QIcon(":/icons/explorer"));
	ui->pushButton_Explorer->setStatusTip(tr("AXEL Explorer"));
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->label_AXEL_Utility_Token4->setVisible(showImmature || showWatchOnlyImmature);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    if (model && model->getOptionsModel()) {
        ...
        connect(ui->blabel_AXEL_Utility_Token, SIGNAL(clicked()), this, SLOT(openMyAddresses()));

    }

    // update the display unit, to not use the default ("AXEL_Utility_Token")
    updateDisplayUnit();
}
  ...
  if (masternodeSync.IsBlockchainSynced() && masternodeSync.IsSynced())
  {
   ...
   std::vector<CMasternode> vMasternodes = mnodeman.GetFullMasternodeMap();
    ...
    ui->graphMN3->setValue(mn3);
		...
		(mn1 == 0) ? roi1 = 0 : roi1 = (0.08*BlockReward*BlockCount24h) / mn1 / COIN;
		(mn2 == 0) ? roi2 = 0 : roi2 = (0.59*BlockReward*BlockCount24h) / mn2 / COIN;
		(mn3 == 0) ? roi3 = 0 : roi3 = (0.13*BlockReward*BlockCount24h) / mn3 / COIN;
		if (chainActive.Height() >= 0) {
			...
			ui->roi_12->setText(mn1 == 0 ? " " : QString::number(1000000 / roi1, 'f', 1).append(" days"));
		...
		CAmount tNodesSumm = mn1 * 1000000 + mn2 * 50000 + mn3 * 5000;
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeAxelAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeAxelAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeAxelAmount = strAnonymizeAxelAmount.remove(strAnonymizeAxelAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAxelAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeAxelAmount * COIN) nMaxToAnonymize = nAnonymizeAxelAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeAxelAmount * COIN) {
        // ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
        //                                       .arg(strAnonymizeAxelAmount));
        strAnonymizeAxelAmount = strAnonymizeAxelAmount.remove(strAnonymizeAxelAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAxelAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::on_pushButton_Website_clicked() {
	QDesktopServices::openUrl(QUrl("https://axel.network/", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Discord_clicked() {
	QDesktopServices::openUrl(QUrl("https://discord.gg/ZxtdRUv", QUrl::TolerantMode));
}
//void OverviewPage::on_pushButton_Telegram_clicked() {
//	QDesktopServices::openUrl(QUrl("https://www.linkedin.com/company/axelunlimited", QUrl::TolerantMode));
//}
void OverviewPage::on_pushButton_Twitter_clicked() {
	QDesktopServices::openUrl(QUrl("https://twitter.com/axelunlimited", QUrl::TolerantMode));
}

void OverviewPage::on_pushButton_Facebook_clicked() {
QDesktopServices::openUrl(QUrl("https://www.facebook.com/AxelUnlimited", QUrl::TolerantMode));
}

void OverviewPage::on_pushButton_Explorer_clicked() {
	if (Params().NetworkID() == CBaseChainParams::MAIN) {
		QDesktopServices::openUrl(QUrl("http://explorer.axel.network/", QUrl::TolerantMode));
	}
	else {
		QDesktopServices::openUrl(QUrl("http://explorer-test.axel.network/", QUrl::TolerantMode));
	}
}
