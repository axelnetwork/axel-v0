...
#define URI_SCHEME "axel_utility_token"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a AXEL Utility Token address (e.g. %1)").arg("AQBxEhGXmWxwpBxlAJqeXqtdUR4MsRZJtV"));
#endif
...
{
    // return if URI is not valid or is no AXEL_Utility_Token: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::AXEL, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert axel_utility_token:// to axel_utility_token:
    //
    //    Cannot handle this later, because axel_utility_token:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::AXEL, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open axel_utility_token.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "AXEL_Utility_Token.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for AXEL_Utility_Token.lnk
...
{
    return GetAutostartDir() / "axel_utility_token.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a axel_utility_token.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=AXEL_Utility_Token\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the AXEL_Utility_Token app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add AXEL_Utility_Token app to startup item list
