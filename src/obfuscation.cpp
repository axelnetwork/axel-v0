...
/* *** BEGIN OBFUSCATION MAGIC - AXEL_Utility_Token **********
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat AXEL_Utility_Token and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 AXEL_Utility_Token per transaction on average.
            */
            if (r <= 10) {
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeAxelAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100AXEL_Utility_Token+1 ( bit on if present )
    // bit 1 - 10AXEL_Utility_Token+1
    // bit 2 - 1AXEL_Utility_Token+1
    // bit 3 - .1AXEL_Utility_Token+1
    // bit 3 - non-denom
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("axel_utility_token-obfuscation");
