...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in axel_utility_token/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in axel_utility_token/kb\n"
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"axel_utility_tokenaddress\"\n"
            "\nReturn information about the given axel_utility_token address.\n"
            "\nArguments:\n"
            "1. \"axel_utility_tokenaddress\"     (string, required) The axel_utility_token address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"axel_utility_tokenaddress\", (string) The axel_utility_token address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: AXEL_Utility_Token address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are axel_utility_token addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) axel_utility_token address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"axel_utility_tokenaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"axel_utility_tokenaddress\"  (string, required) The axel_utility_token address to use for the signature.\n"
