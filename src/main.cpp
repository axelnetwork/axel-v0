...
#if defined(NDEBUG)
#error "AXEL_Utility_Token cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uaxel_utility_token) are considered zero fee (for relaying and mining)
...
CAmount GetBlockValue(int nHeight)
{
  // anti instamine
	...
	else if (nHeight > 500 && nHeight <= 478080) {
		nSubsidy = 15 * COIN;
	}
	else {
		nSubsidy = 61 * COIN;
...
int64_t GetMasternodePayment(int nHeight, unsigned mnlevel, int64_t blockValue)
{
	...
	{
	case 1:
		return blockValue * 0.08;

	case 2:
		return blockValue * 0.59;
...
{
    RenameThread("axel_utility_token-scriptch");
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    //     //
    //     // if (stake->IsZAXEL() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
    //     //     return state.DoS(100, error("%s: staked zAXEL fails context checks", __func__));
    //
    //     uint256 hash = block.GetHash();
    //     if(!mapProofOfStake.count(hash)) // add to mapProofOfStake
    //         mapProofOfStake.insert(make_pair(hash, hashProofOfStake));
