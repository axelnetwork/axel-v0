...
#include "config/axel_utility_token-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("axel_utility_token-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "axel_utility_token.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "axel_utility_tokend.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 15319, 25318));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in AXEL_Utility_Token/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in AXEL_Utility_Token/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, proxy, axel_utility_token, (obfuscation, swiftx, masternode, mnpayments)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in AXEL_Utility_Token/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all AXEL_Utility_Token specific functionality (Masternodes, Obfuscation, SwiftX) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:15319"));
    ...
    strUsage += HelpMessageOpt("-anonymizeaxel_utility_tokenamount=<n>", strprintf(_("Keep N AXEL_Utility_Token anonymized (default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 9998, 25318));
...
std::string LicenseInfo()
{
    return FormatParagraph(_("Copyright (C) 2009-2014 The Bitcoin Core Developers")) + "\n" +
           ...
           "\n" +
           FormatParagraph(_("Copyright (C)  2019  The AXEL Utility Token Core Developers")) + "\n" +
           "\n" +
...
{
    RenameThread("axel_utility_token-loadblk");
...
/** Sanity checks
 *  Ensure that AXEL_Utility_Token is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. AXEL_Utility_Token Core is shutting down."));
...
#endif
    // Make sure only a single AXEL_Utility_Token process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. AXEL_Utility_Token Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("AXEL_Utility_Token version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of AXEL_Utility_Token Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart AXEL_Utility_Token Core to complete") << "\n";
    ...
    nAnonymizeAxelAmount = GetArg("-anonymizeaxel_utility_tokenamount", 0);
    if (nAnonymizeAxelAmount > 999999) nAnonymizeAxelAmount = 999999;
    if (nAnonymizeAxelAmount < 2) nAnonymizeAxelAmount = 2;
    ...
    LogPrintf("Anonymize AXEL_Utility_Token Amount %d\n", nAnonymizeAxelAmount);
    ...
    /* Denominations
       ...
       For example:
       1AXEL_Utility_Token+1000 == (.1AXEL_Utility_Token+100)*10
       10AXEL_Utility_Token+10000 == (1AXEL_Utility_Token+1000)*10
    */
