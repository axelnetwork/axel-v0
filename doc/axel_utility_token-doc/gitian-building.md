...
*Setup instructions for a gitian build of AXEL_Utility_Token using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the AXEL_Utility_Token
...
to axel_utility_token-crypto.com.
...
- [Building AXEL_Utility_Token](#building-axel_utility_token)
...
Clone the git repositories for axel_utility_token and gitian and then checkout the axel_utility_token version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/axel_utility_token-crypto/axel_utility_token.git
...
Follow the instructions in [doc/release-process.md](release-process.md) in the axel_utility_token repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building AXEL_Utility_Token
----------------

To build AXEL_Utility_Token (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the axel_utility_token repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/axel_utility_token/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/axel_utility_token-crypto/axel_utility_token
...
For example:
```bash
URL=https://github.com/crowning-/axel_utility_token.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit axel_utility_token=${COMMIT} --url axel_utility_token=${URL} ../axel_utility_token/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit axel_utility_token=${COMMIT} --url axel_utility_token=${URL} ../axel_utility_token/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit axel_utility_token=${COMMIT} --url axel_utility_token=${URL} ../axel_utility_token/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/axel_utility_token-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/axel_utility_token-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/axel_utility_token-build.assert
```
...
[axel_utility_token/gitian.sigs](https://github.com/axel_utility_token-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
